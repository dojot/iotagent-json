...
function areMatchingTopics(templateTopic, receivedTopic) {
    ...
    for (var i = 0, j = 0; i < templateTopic.length, j < receivedTopic.length; i++, j++) {
        ret &= ((templateTopic === '+') || (templateTopic === '#') || (templateTopic === receivedTopic));
        if (ret == false) {
...
function mqttMessageHandler(topic, message) {
    ...
    function processDeviceMeasure(error, device) {
        if (error) {
            ...
            // if (topicInformation[3] === 'configuration' && topicInformation[4] === 'commands' && translatedParsedMessage) {
            //     manageConfigurationRequest(apiKey, deviceId, device, translatedParsedMessage);
            ...
            //     commandHandler.updateCommand(apiKey, deviceId, device, translatedParsedMessage);
            // } else if (translatedParsedMessage && typeof translatedParsedMessage === 'object') {
            //     multipleMeasures(apiKey, deviceId, device, translatedParsedMessage);
            // } else {
            ...
            if (mqtt_specs == undefined) {
                config.getLogger().warn(context, 'No topic found for device %s', device.name);
                return;
            }

            var matchingTopic = false;
            ...
            if (mqtt_specs.configuration != undefined) {
                if (mqtt_specs.configuration instanceof Array) {
                    for (var currTopic = 0; currTopic < mqtt_specs.configuration.length; currTopic++) {
                        var configurationTopic = mqtt_specs.configuration[currTopic].topic.split(':')[2].split('/');
                        matchingTopic = areMatchingTopics(configurationTopic, topicInformation);
                        if (matchingTopic == true) {
                            break;
                        }
                    }
                } else {
                    var configurationTopic = mqtt_specs.configuration.topic.split(':')[2].split('/');
                    matchingTopic = areMatchingTopics([configurationTopic], topicInformation);
                }
            }
            ...
            if (matchingTopic == false) {
                ...
                if (attributeName == undefined) {
                    multipleMeasures('', deviceId, device, translatedParsedMessage);
                } else {
                    singleMeasure('', deviceId, attributeName, device, translatedParsedMessage);
                }
            } else {
    ...
    function listDevicesInternal(error, result) {
        for (var i = 0; i < result.count; i++) {
            if (result.devices[i].name == deviceId) {
                iotAgentLib.retrieveDevice(result.devices[i].id, '', processDeviceMeasure);
            }
        }
        if (result.count == 100) {
            offset += result.count;
...
function deviceNotificationCallback(event, device) {
    ...
    // - topic: {TRANSP}:mqtt:{TOPIC}
    // For now we are ignoring transport.
    var mqtt_specs = device.internalAttributes;
    var topics = [];
    if (mqtt_specs != undefined) {
